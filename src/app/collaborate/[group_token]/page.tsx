...
export default function GroupContributionPage() {
  ...
  useEffect(() => {
    const fetchBoard = async () => {
      try {
        const res = await fetch(`http://localhost:8000/api/collaborate/${groupToken}`);
        if (res.ok) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (mediaFile) {
        ...
        const uploadRes = await fetch('http://localhost:8000/api/storage/upload', {
          method: 'POST',
          body: formData,
        });
      ...
      const res = await fetch(`http://localhost:8000/api/collaborate/${groupToken}/contribute`, {
        method: 'POST',
